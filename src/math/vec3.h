...
class vec3 {
    public:
        ...
        vec3() : e{0, 0, 0}, x(e[0]), y(e[1]), z(e[2]) {}
        vec3(double lx, double ly, double lz)
            : e{lx, ly, lz}, x(e[0]), y(e[1]), z(e[2]) {}
        ...
        // simple rejection method to get random vector inside a unit sphere
        static vec3 random_unit_vector() {
            while (true) {
                auto p = vec3::random(-1, 1);
                auto lensq = p.length_squared();
                // 1e-160 is for floating point abstraction leak so we don't
                // accidentally give out -inf/+inf vector
                if (1e-160 < lensq && lensq <= 1)
                    return p / sqrt(lensq);
            }
        }

        // same hemisphere check as checking rebound rays on intersect
        ...
        static vec3 refract(const vec3 &uv, const vec3 &n,
