...
void bouncing_spheres() {
    ...
    auto offset = vec3(0.15, 0.15, 0.15);
        ...
        for (int b = -11; b < 11; b++) {
            ...
            if ((center - point3(4, 0.2, 0)).length() > 0.9) {
                ...
                if (choose_mat < 0.8) {
                    ...
                    // sphere_material));
                    world.add(
                        box(center - offset, center + offset, sphere_material));
    ...
    auto large_offset = vec3(.7, .7, .7);
    auto material1 = make_shared<dielectric>(1.5);
    // world.add(make_shared<sphere>(point3(0, 1, 0), 1.0, material1));
    world.add(box(point3(0, 1, 0) - large_offset,
                  point3(0, 1, 0) + large_offset, material1));

    auto material2 = make_shared<lambertian>(color(0.4, 0.2, 0.1));
    // world.add(make_shared<sphere>(point3(-4, 1, 0), 1.0, material2));
    world.add(box(point3(-4, 1, 0) - large_offset,
                  point3(-4, 1, 0) + large_offset, material2));

    auto material3 = make_shared<metal>(color(0.7, 0.6, 0.5), 0.0);
    // world.add(make_shared<sphere>(point3(4, 1, 0), 1.0, material3));
    world.add(box(point3(4, 1, 0) - large_offset,
                  point3(4, 1, 0) + large_offset, material3));
...
void checkered_spheres() {
    ...
    auto checker =
        make_shared<checker_texture>(.32, color(.2, .3, .1), color(.9, .9, .9));
...
void earth() {
    auto earth_texture = make_shared<image_texture>("res/test");
...
void example_sc() {
    ...
    auto material_ground = make_shared<lambertian>(color(0.5, 0.5, 0.8));
...
void simple_light() {
    ...
    auto tex_mat = make_shared<lambertian>(tex);

    auto uw_tex = make_shared<image_texture>("res/UW.png");
    auto uw_mat = make_shared<lambertian>(uw_tex);

    auto pertext = make_shared<noise_texture>(4);
    world.add(
        make_shared<sphere>(point3(0, -1000, 0), 1000,
                            make_shared<lambertian>(color(0.5, 0.5, 0.8))));
    world.add(make_shared<sphere>(point3(0, 2, 0), 2,
                                  make_shared<lambertian>(pertext)));
    world.add(make_shared<sphere>(point3(0, 2, 5), 2, tex_mat));
    auto box_center = point3(0, 2, 10);
    auto box_offset = point3(2, 2, 2);
    world.add(box(box_center - box_offset, box_center + box_offset, uw_mat));

    auto difflight = make_shared<diffuse_light>(color(10, 10, 10));
    world.add(make_shared<quad>(point3(3, 1, -4), vec3(5, 0, 0), vec3(0, 5, 0),
                                difflight));
